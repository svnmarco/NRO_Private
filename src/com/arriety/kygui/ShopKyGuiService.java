...
//                msg.writer().writeByte(0);
        ...
        switch (action) {
            ...
            case 2: // nhận tiền
                ...
                if (it.goldSell > 0) {
                    ...
                    pl.inventory.ruby += it.rubySell - it.rubySell * 10 / 100;
                }
                if (ShopKyGuiManager.gI().listItem.remove(it)) {
                    Service.gI().sendMoney(pl);
                    ShopKyGuiManager.gI().save();
    ...
    public List<ItemKyGui> getItemCanKiGui(Player pl) {
        ...
        pl.inventory.itemsBag.stream().filter((it) -> (it.isNotNullItem()
                && (it.isItemKyGui()))).forEachOrdered((it) -> {
    ...
    public byte getTabKiGui(Item it) {
        if (it.template.type == 0 || it.template.type == 1) {
            return 0;
        } else if ((it.template.type == 3 || it.template.type == 4 || it.template.type == 2)) {
            return 1;
        } else if (it.template.type == 24 || it.template.type == 21 || it.template.type == 11 || it.template.type == 72 || it.template.type == 74) {
            return 2;
        } else if (it.template.type != 0 || it.template.type != 1 || it.template.type != 2 || it.template.type != 3 || it.template.type != 4 || it.template.type != 5 || it.template.type != 11 || it.template.type != 21 || it.template.type != 24  || it.template.type != 72  || it.template.type != 74) {
            return 3;
        }
        return 0;
    }
